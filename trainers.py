...
class DPOPTrainer(PairedPreferenceTrainer):
    def loss(self,
             policy_chosen_logps: torch.FloatTensor,
             policy_rejected_logps: torch.FloatTensor,
             reference_chosen_logps: torch.FloatTensor,
        ...
        lambda_max = self.config.loss.lamda * torch.clamp(reference_chosen_logps - policy_chosen_logps, min=0)

        losses = -F.logsigmoid(self.config.loss.beta * (logits - lambda_max))
...
class TDPOTrainer(BasicTrainer):
    """A trainer for any loss that uses paired preference, like DPO."""
    def concatenated_inputs(self, batch: Dict[str, Union[List, torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
        """Concatenate the chosen and rejected inputs into a single tensor. The first half is chosen outputs, the second half is rejected.
